/*
 * $Id: XMRTPH263Packetizer.c,v 1.10 2008/10/11 18:56:55 hfriederich Exp $
 *
...
ComponentResult XMRTPH263Packetizer_Open(XMRTPH263PacketizerGlobals globals, ComponentInstance self)
{
...
ComponentResult XMRTPH263Packetizer_Close(XMRTPH263PacketizerGlobals globals, ComponentInstance self)
{
  if (globals) {
    free(globals);
  }
...
ComponentResult XMRTPH263Packetizer_Target(XMRTPH263PacketizerGlobals globals, ComponentInstance target)
{
...
ComponentResult XMRTPH263Packetizer_Initialize(XMRTPH263PacketizerGlobals globals, SInt32 inFlags)
{
  return noErr;
}

ComponentResult XMRTPH263Packetizer_PreflightMedia(XMRTPH263PacketizerGlobals globals, OSType inMediaType,
                                                   SampleDescriptionHandle inSampleDescription)
{	
  return noErr;
}

ComponentResult XMRTPH263Packetizer_Idle(XMRTPH263PacketizerGlobals globals, SInt32 inFlags, SInt32 *outFlags)
{
  *outFlags = 0;
  return noErr;
}
...
{	
  ...
  RTPPBEndPacketGroup(globals->packetBuilder, 0, packetGroupRef);
	
  *outFlags = 0;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_Flush(XMRTPH263PacketizerGlobals globals, SInt32 inFlags, SInt32 *outFlags)
{
  *outFlags = 0;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_Reset(XMRTPH263PacketizerGlobals globals, SInt32 inFlags)
{
  return noErr;
}

ComponentResult XMRTPH263Packetizer_SetInfo(XMRTPH263PacketizerGlobals globals, OSType inSelector, const void *ioParams)
{
  printf("SetInfo called\n");
  return paramErr;
}

ComponentResult XMRTPH263Packetizer_GetInfo(XMRTPH263PacketizerGlobals globals, OSType inSelector, void *ioParams)
{
  printf("GetInfo called\n");
  return paramErr;
}

ComponentResult XMRTPH263Packetizer_SetTimeScale(XMRTPH263PacketizerGlobals globals, TimeScale inTimeScale)
{
  globals->timeScale = inTimeScale;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_GetTimeScale(XMRTPH263PacketizerGlobals globals, TimeScale *outTimeScale)
{
  *outTimeScale = globals->timeScale;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_SetTimeBase(XMRTPH263PacketizerGlobals globals, TimeBase inTimeBase)
{
  globals->timeBase = inTimeBase;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_GetTimeBase(XMRTPH263PacketizerGlobals globals, TimeBase *outTimeBase)
{
  *outTimeBase = globals->timeBase;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_HasCharacteristic(XMRTPH263PacketizerGlobals globals, OSType inSelector, Boolean *outHasIt)
{
  printf("HasCharacteristic called\n");
  return paramErr;
}

ComponentResult XMRTPH263Packetizer_SetPacketBuilder(XMRTPH263PacketizerGlobals globals, ComponentInstance inPacketBuilder)
{
  globals->packetBuilder = inPacketBuilder;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_GetPacketBuilder(XMRTPH263PacketizerGlobals globals, ComponentInstance *outPacketBuilder)
{
  *outPacketBuilder = globals->packetBuilder;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_SetMediaType(XMRTPH263PacketizerGlobals globals, OSType inMediaType)
{
  if (inMediaType != 'h263') {
    return paramErr;
  }
...
ComponentResult XMRTPH263Packetizer_GetMediaType(XMRTPH263PacketizerGlobals globals, OSType *outMediaType)
{
  *outMediaType = 'h263';
  return noErr;
}

ComponentResult XMRTPH263Packetizer_SetMaxPacketSize(XMRTPH263PacketizerGlobals globals, UInt32 inMaxPacketSize)
{
  globals->maxPacketSize = inMaxPacketSize;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_GetMaxPacketSize(XMRTPH263PacketizerGlobals globals, UInt32 *outMaxPacketSize)
{
  *outMaxPacketSize = globals->maxPacketSize;
  return noErr;
}

ComponentResult XMRTPH263Packetizer_SetMaxPacketDuration(XMRTPH263PacketizerGlobals globals, UInt32 inMaxPacketDuration)
{
  printf("SetMaxPacketDuration called\n");
  return paramErr;
}

ComponentResult XMRTPH263Packetizer_GetMaxPacketDuration(XMRTPH263PacketizerGlobals globals, UInt32 *outMaxPacketDuration)
{
  printf("GetMaxPacketDuration called\n");
  return paramErr;
}

ComponentResult XMRTPH263Packetizer_DoUserDialog(XMRTPH263PacketizerGlobals globals, ModalFilterUPP inFilterUPP, Boolean *canceled)
{
...
ComponentResult XMRTPH263Packetizer_GetSettingsAsText(XMRTPH263PacketizerGlobals globals, Handle *text)
{
                                           ...
                                           Boolean isLastGOB)
{
  ...
  while (remainingLength > maxPacketSize) {
    ...
    if (mbIsEncoded == true) {
      ...
      if (mbType == MB_TYPE_1 || mbType == MB_TYPE_4) {
        UInt8 dquant;
      ...
      if (mbType == MB_TYPE_0 || mbType == MB_TYPE_1) {
        UInt8 hmvd;
        ...
        if ((hmv & 0x40) == 0 && (hmv >= 0x20)) {
          hmv |= 0xc0;
        } else if ((hmv & 0x40) != 0 && (hmv < 0x60)) {
          hmv &= 0x3f;
        }
        ...
        if ((vmv & 0x40) == 0 && (vmv >= 0x20)) {
          vmv |= 0xc0;
        } else if ((vmv & 0x40) != 0 && (vmv < 0x60)) {
          vmv &= 0x3f;
        }
      } else {
        hmv = 0;
 ...
 * This function registers the XMRTPH263Packetizer component so that
...
{	
  ...
  // Registering the Component
  ComponentRoutineUPP componentEntryPoint = NewComponentRoutineUPP((ComponentRoutineProcPtr)&XMRTPH263Packetizer_ComponentDispatch);
