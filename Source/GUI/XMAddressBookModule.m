...
- (IBAction)call:(id)sender
{
  ...
  XMAddressBookRecord *record = [[XMAddressBookManager sharedInstance] recordForPerson:person identifier:identifier isPhoneNumber:isPhoneNumber];
...
- (IBAction)editAddress:(id)sender
{
  ...
  XMAddressBookRecord *record = [[XMAddressBookManager sharedInstance] recordForPerson:person identifier:identifier isPhoneNumber:NO];
...
- (IBAction)newAddress:(id)sender
{
  ...
  XMAddressBookRecord *record = [[XMAddressBookManager sharedInstance] recordForPerson:person identifier:nil isPhoneNumber:NO];
  [self _editRecord:record];
}
...
- (IBAction)addressBookDoubleAction:(id)sender
{
  ...
  if ([self _canEditSelectedRecord]) {
...
- (void)_editRecord:(XMAddressBookRecord *)record
{
  ...
  if (label == nil || [label isEqualToString:@"H.323"] || [label isEqualToString:@"SIP"]) {
    tag = XM_DEFAULT_TAG;
    label = @"";
    ...
    tag = XM_OTHER_TAG;
    label = @"";
  } else if (label != nil && ![label isEqualToString:@"H.323"] && ![label isEqualToString:@"SIP"]) {
    tag = XM_CUSTOM_TAG;
  }
